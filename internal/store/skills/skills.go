...
// syncSkillFiles fetches and saves skill files from GitHub
func (s *SQLSkillStore) syncSkillFiles(skillID string) error {
	// Type assert to access GetSkillFiles method
	type skillFilesFetcher interface {
		GetSkillFiles(skillID string) ([]model.SkillFile, error)
	}

	fetcher, ok := s.githubStore.(skillFilesFetcher)
	if !ok {
		// GitHub store doesn't support file fetching
		return nil
	}

	// Fetch files from GitHub
	files, err := fetcher.GetSkillFiles(skillID)
	if err != nil {
		return err
	}

	// Delete existing files for this skill
	if err := s.DeleteSkillFiles(skillID); err != nil {
		return fmt.Errorf("failed to clear existing files: %w", err)
	}

	// Save each file
	for _, file := range files {
		if err := s.SaveSkillFile(&file); err != nil {
			return fmt.Errorf("failed to save file %s: %w", file.Path, err)
		}
	}

	log.Printf("[SQLite] Synced %d files for skill '%s'", len(files), skillID)
	return nil
}
